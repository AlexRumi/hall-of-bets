...
**apuestas**: id, user_id, fecha, casa, stake, tipo_fondos (`real`/`freebet`), categoria (`apuestas`/`entretenimiento`), resultado (`pendiente`/`ganada`/`perdida`/`nula`), selecciones (jsonb: una o varias filas — más de una es una combinada), creado_en
...
18. **Dashboard de Estadísticas** — KPIs (beneficio, ROI, yield, win rate, bankroll, cuota media, nº apuestas, beneficio mensual), evolución (línea, con filtros 7d/30d/90d/año/histórico), beneficio mensual (barras), distribución Ganada/Perdida/Nula/Pendiente (circular), rachas (actual/mejor/peor), mejor y peor apuesta, ROI por deporte, ROI por casa, beneficio por rango de cuotas, calendario de actividad, insights automáticos. Añade el campo **deporte** al registrar apuestas: lista cerrada corta — Fútbol, Baloncesto, Tenis, eSports, Otro.

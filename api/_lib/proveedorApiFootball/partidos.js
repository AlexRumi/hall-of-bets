// EN PAUSA desde la migración a GOAL API (ver CHANGELOG.md): la cuenta
// de API-Football está suspendida sin fecha de reactivación, y GOAL API
// cubre hoy las mismas 45 competiciones con muchísimo más margen de
// peticiones. Este archivo NO está enlazado a ninguna ruta activa —
// vive aquí (en vez de en api/, donde Vercel lo desplegaría como
// endpoint) a propósito, completo y sin resumir, por si la cuenta de
// API-Football se reactiva algún día y se quiere volver a usar, sola o
// combinada con GOAL API. El endpoint activo hoy es api/partidos.js
// (llama a GOAL API). Import corregido tras el traslado: un nivel más
// arriba (este archivo ya vivía en api/_lib/, ahora en un subdirectorio
// suyo).
//
// --- Comentarios originales, tal cual, de cuando esto era el endpoint activo ---
//
...
import { permiteLlamadaApiFootball } from "../limitadorApiFootball.js";

...
- Limpieza diaria del chat de Telegram, cada mañana entre las 8 y las 9 (hora de España) (`api/telegram-limpieza.js`, mismo mecanismo de cron externo que el aviso de arriba, con su propio `LIMPIEZA_CRON_SECRET`): un bot solo puede borrar sus propios mensajes (nunca lo que escribe el usuario ni los toques a los botones del teclado), así que esto borra todo lo que manda el bot EXCEPTO la confirmación de `/start` (tipo `start`, nunca se borra — hacerlo colapsaría el teclado personalizado) y los mensajes con botón "Ver apuesta" de una apuesta que sigue pendiente (tipos `listado`/`aviso`, para que los botones del teclado se refieran a algo real). Las apuestas ya resueltas (tipos `registro`/`resuelta`, y `listado`/`aviso` ya marcados con `resuelta_en`) se borran siempre — decidido con el usuario tras detectar que Todas/Apuestas/Entretenimiento solo listan lo pendiente: el chat queda limpio de verdad cada mañana, los datos reales no se tocan (siguen en la app). La franja horaria se calcula con `Intl.DateTimeFormat`, igual que el punto de abajo.

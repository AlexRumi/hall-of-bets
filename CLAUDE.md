...
- Aviso de "partido terminado" (`api/telegram-avisos.js`): manda un mensaje de Telegram ("⚽ X ha terminado, ya puedes confirmar tu apuesta") en cuanto detecta que un partido con apuestas pendientes ha acabado — con botón directo a la Mini App de cada apuesta afectada. No usa el cron propio de Vercel (el plan Hobby solo deja programar una vez al día, `docs/cron-jobs/usage-and-pricing`) — lo dispara un cron EXTERNO gratuito, [cron-job.org](https://cron-job.org), cada 15 min, contra `https://hall-of-bets.vercel.app/api/telegram-avisos?secret=<AVISOS_CRON_SECRET>` (variable de entorno nueva, cadena aleatoria propia — no confundir con `TELEGRAM_WEBHOOK_SECRET`, que solo Telegram conoce). Agrupa por `partidoId` (un partido puede estar en varias apuestas pendientes a la vez) para gastar como mucho 1 llamada a `api/partido.js` por partido, nunca por apuesta; la columna `notificado` de `resultados_partidos` evita avisar dos veces del mismo partido. Coste típico: 1 llamada a la API de fútbol por partido pendiente distinto en todo el día, no por revisión del cron.

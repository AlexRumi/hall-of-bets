...
- Aviso de "apuesta resuelta" (`api/telegram-resuelta.js`): en cuanto una apuesta deja de estar pendiente (la marcas Ganada/Perdida/Nula/Cash Out, desde la app o la Mini App), llega un resumen con el marcador de cada partido y el beneficio. Mismo mecanismo que el aviso de registro — un SEGUNDO Database Webhook en el mismo sitio (Integrations > Database Webhooks), mismo `REGISTRO_WEBHOOK_SECRET`, pero evento `UPDATE` en vez de `INSERT`; el propio código descarta cualquier `UPDATE` que no sea justo la transición de `pendiente` a otra cosa (editar una apuesta ya resuelta, o cualquier otro cambio, no dispara nada). Deliberadamente NO deduce el resultado del marcador — eso lo decides tú a mano con "Modificar" (Pendiente/Ganada/Perdida/Nula, la app nunca lo calcula sola, la mayoría de mercados no se pueden derivar así) — este aviso solo confirma lo que ya has terminado de marcar.
- Botón de los avisos actualizado al resolver (`api/_lib/telegramMensajes.js`): cada mensaje que manda el bot (con botón "Ver apuesta" o sin él) guarda su `chat_id`/`message_id` y un `tipo` (`start`/`listado`/`aviso`/`registro`/`resuelta`) en la tabla `telegram_mensajes`. Cuando `api/telegram-resuelta.js` se dispara, edita el botón de TODOS los mensajes con botón guardados de esa apuesta (✅ Ganada / ❌ Perdida / ➖ Nula / 💰 Cash Out) y los marca con `resuelta_en` (ya no borra las filas, para que la limpieza diaria de abajo sepa qué mensajes de Telegram le quedan por borrar).
- Limpieza diaria del chat de Telegram, cada mañana a las 9:00 (hora de España) (`api/telegram-limpieza.js`, mismo mecanismo de cron externo que el aviso de arriba, con su propio `LIMPIEZA_CRON_SECRET`): un bot solo puede borrar sus propios mensajes (nunca lo que escribe el usuario ni los toques a los botones del teclado), así que esto borra todo lo que manda el bot EXCEPTO la confirmación de `/start` (tipo `start`, nunca se borra — hacerlo colapsaría el teclado personalizado) y los mensajes con botón "Ver apuesta" de una apuesta que sigue pendiente (tipo `listado`, único que sigue llevando botón editable — para que los botones del teclado se refieran a algo real). El resto (tipos `aviso`/`registro`/`resuelta`, y `listado` ya marcado con `resuelta_en`) se borra siempre — decidido con el usuario tras detectar que Todas/Apuestas/Entretenimiento solo listan lo pendiente: el chat queda limpio de verdad cada mañana, los datos reales no se tocan (siguen en la app). El aviso "📝 Ya puedes confirmarla" (`api/telegram-avisos.js`) ya no lleva botón "Ver apuesta" (petición directa, texto plano) — el aviso "apuesta resuelta" (`api/telegram-resuelta.js`) EDITA su propio mensaje en cada corrección de resultado en vez de mandar uno nuevo (bug real: la pastilla de partido cicla Pendiente→Ganada→Perdida→Nula y el ciclo siempre pasa primero por Ganada, así que el aviso podía llegar con el resultado equivocado en el primer clic — ahora un único mensaje por apuesta, siempre con el resultado más reciente). La franja horaria se calcula con `Intl.DateTimeFormat`, igual que el punto de abajo.

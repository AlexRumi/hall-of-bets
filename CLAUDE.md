...
- Botón de los avisos actualizado al resolver (`api/_lib/telegramMensajes.js`): cada mensaje con botón "Ver apuesta"/"Abrir apuesta" (de `/pendientes` y del aviso de "lista para confirmar") guarda su `chat_id`/`message_id` en la tabla `telegram_mensajes`. Cuando `api/telegram-resuelta.js` se dispara, edita el botón de TODOS los mensajes guardados de esa apuesta (✅ Ganada / ❌ Perdida / ➖ Nula / 💰 Cash Out) y borra las filas — ya no hacen falta.

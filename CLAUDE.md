...
- Gráficas: recharts
- Iconos: lucide-react
- Despliegue: GitHub (`AlexRumi/hall-of-bets`, rama `main`) + Vercel (`hall-of-bets.vercel.app`), auto-deploy en cada push a `main`. Vercel necesita las variables de entorno `VITE_SUPABASE_URL`, `VITE_SUPABASE_PUBLISHABLE_KEY`, `GOAL_API_KEY` y `SUPABASE_SERVICE_ROLE_KEY` (Project Settings > Environment Variables) — ver comentarios de cada una en `.env.example`; en local van en `.env.local` (no se sube a git). `API_FOOTBALL_KEY` ya no la necesita ningún endpoint activo, se puede dejar o quitar de Vercel sin efecto
- Proveedor de datos de partidos: **GOAL API** desde el 2026-09-03 (migración, ver CHANGELOG.md) — antes API-Football, cuya cuenta se suspendió el 2026-09-02 sin fecha de reactivación. El código de API-Football se dejó EN PAUSA, no borrado, en `api/_lib/proveedorApiFootball/` (por si la cuenta se reactiva algún día, sola o combinada con GOAL API). Los 3 endpoints activos (`api/partidos.js`, `api/jugadores.js`, `api/partido.js`) llaman a GOAL API vía `api/_lib/goalApi.js` (helper de peticiones + mapeo de las 45 competiciones conectadas). Limitador propio (`api/_lib/limitadorGoalApi.js`): a diferencia del de API-Football (ráfaga por minuto, ya no es un riesgo real con GOAL — comprobado con 8 peticiones simultáneas sin bloqueo, ~200/min de margen), aquí solo importa no acercarse a las 1000 peticiones/día — contador diario atómico en Supabase (`limitador_goal_api`, `SELECT ... FOR UPDATE`). El limitador de API-Football (`api/_lib/limitadorApiFootball.js`, tabla `limitador_api_football`) sigue en el repo sin usarse, para cuando se reactive ese código
- Caché permanente de plantillas de equipo (`api/jugadores.js`, tabla `plantillas_equipos`, columna `equipo_id` ahora `text` para aceptar los ids-string de GOAL API): la primera vez que se pide la plantilla de un equipo se guarda en Supabase durante 30 días — ni tú, ni tu amigo, ni un dispositivo nuevo, vuelven a gastar una llamada real por ese equipo hasta que caduque. Antes solo se cacheaba en memoria del navegador (por pestaña), así que cada dispositivo/sesión nueva la pedía de cero. 30 días (no permanente del todo) porque un equipo puede fichar/vender jugadores en el mercado de invierno/verano.
...
5. Grupo "Competición Internacional" en el buscador de partidos (UEFA
   ...
   implementar. Antes de retomarlo, valorar cobertura de cada una en
   GOAL API (mismo paso de verificación por `leagues?search=` que ya se
   hizo con el resto de ligas conectadas, ver `api/_lib/goalApi.js`).
